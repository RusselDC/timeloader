...
from api.controller.UserController import AuthUserControllerDep, NonAuthUserControllerDep
...
@user.post("/login")
def create_page(
    controller: NonAuthUserControllerDep,
    data: UserLoginData,
):
    try:
...
def verify_token(jwt_service: JWTServiceDep):
    try:
